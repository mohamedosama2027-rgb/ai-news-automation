...
توجد اختبارات تكامل اختيارية داخل `tests/`. معظمها يستدعي APIs حقيقية وقد يستهلك quota أو ينشر على Facebook؛ شغّلها فقط بعد التأكد من المتغيرات. `MAX_POSTS_PER_RUN` افتراضيًا `1`، وكل تشغيل مجدول ينشر بوستًا واحدًا. يحتفظ سجل `data/published-news.json` بكل المنشورات والصور السابقة لمنع إعادة استخدامها.
...
يعمل `daily.yml` يوميًا عشر مرات بتوقيت القاهرة (`Africa/Cairo`): 8:17 و9:23 و10:29 و11:37 صباحًا، ثم 1:43 و3:49 و5:07 و6:13 و8:19 و10:31 مساءً. كل وقت له تركيز موضوعي مختلف، ويظل التشغيل على بوست واحد. أما `automation.yml` فهو للتشغيل اليدوي فقط ويختار موضوعًا تلقائيًا. الـ workflowين يستخدمان مجموعة concurrency واحدة لمنع تشغيل نشرين بالتوازي، ولكل تشغيل حد أقصى 30 دقيقة.
