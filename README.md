...
أداة Node.js تجمع أخبار الذكاء الاصطناعي والتكنولوجيا والروبوتات من Google News وBing، وتكتشف أدوات AI المجانية أو مفتوحة المصدر من Product Hunt. تنقح المحتوى وتمنع التكرار، ثم يستخدم Gemini لكتابة منشورات Facebook باللهجة المصرية مع رابط مباشر للأداة. تستخدم منشورات الأدوات صورة المنتج من Product Hunt؛ وتستخدم الأخبار صور Pexels افتراضيًا، مع خيارات توليد الصور عبر Gemini أو مقارنتها بعد تفعيل الفوترة.

ينشر الجدول اليومي 10 مرات: 5 منشورات لأحدث أخبار AI و5 تسلط الضوء على أدوات AI من Product Hunt. تتنوع الأدوات بحسب المتاح بين البرمجة والصور والفيديو والصوت والكتابة والبيانات والأمن والصحة والتعليم والبحث والتمويل والقانون والزراعة والمناخ والحكومة والأعمال والسفر والألعاب والتجارة والعقارات وغيرها من المجالات.

## ماذا يفعل المشروع؟

1. يجلب أخبار AI الحديثة من Google News وBing، ويكتشف منتجات AI المجانية أو مفتوحة المصدر من Product Hunt.
...
7. يستخدم صورة المنتج من Product Hunt لمنشورات الأدوات، ويبحث عن صور الأخبار في Pexels افتراضيًا ويقيّمها عبر Gemini؛ ويمكن توليد صورة بديلة أو مقارنتها بـPexels.
...
| `PRODUCT_HUNT_API_TOKEN` | نعم لاكتشاف الأدوات | Product Hunt developer token؛ يقرأ أحدث المنتجات خلال 7 أيام ويشترط إشارة واضحة إلى أنها مجانية أو مفتوحة المصدر |
...
تُكتشف أدوات AI من Product Hunt باستخدام `PRODUCT_HUNT_API_TOKEN`. يتصفح النظام حتى 5 صفحات من أحدث المنتجات المنشورة خلال 7 أيام، ويشترط وجود إشارة واضحة إلى المجانية أو open source في الوصف أو الوسوم؛ فالـAPI لا يوفّر حقل تسعير رسميًا. يصنّف المنتجات عبر مجالات AI المختلفة، ويستخدم صورة المنتج من Product Hunt مباشرةً. إذا غابت الصورة أو تعذّر تنزيلها، يستخدم مصدر الصور المعتاد كبديل. يضيف رابط الموقع المباشر إلى المنشور ورابط Product Hunt كمصدر.

يمنع سجل `data/published-news.json` تكرار الأداة المنشورة باستخدام اسمها بعد توحيده ورابطها وسجل Product Hunt. كل تشغيل يومي ينشر منشورًا واحدًا؛ وإذا لم تتوفر صورة Product Hunt يعمل مصدر الصور البديل قبل تخطي المنشور.
...
توجد اختبارات تكامل اختيارية داخل `tests/`. معظمها يستدعي APIs حقيقية وقد يستهلك quota؛ واختبارا Facebook ينشران منشورًا فعليًا على الصفحة. `MAX_POSTS_PER_RUN` افتراضيًا `1`، وكل تشغيل مجدول ينشر منشورًا واحدًا. يحتفظ سجل `data/published-news.json` بالمنشورات والأدوات والصور السابقة لمنع إعادة استخدامها.
...
يعمل `daily.yml` يوميًا عشر مرات بتوقيت القاهرة (`Africa/Cairo`) وينشر منشورًا واحدًا في كل موعد:

- **أخبار AI (5):** 8:17 و9:23 و10:29 صباحًا، و3:49 و8:19 مساءً.
- **أدوات Product Hunt (5):** 11:37 صباحًا، و1:43 و5:07 و6:13 و10:31 مساءً.
